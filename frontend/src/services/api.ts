...
export const deckAPI = {
  ...
  saveDeck: async (deckData: {
    ...
    // deckId가 있으면 수정, 없으면 생성
    if (deckData.deckId) {
      const response = await api.put(`/decks/${deckData.deckId}`, deckData);
      return response;
    } else {
      const response = await api.post('/decks', deckData);
      return response;
  ...
  saveDeckWithCards: async (deckData: {
    ...
    // deckId가 있으면 수정, 없으면 생성
    if (newFormat.deckId) {
      const response = await api.put(`/decks/${newFormat.deckId}`, newFormat);
      return response;
    } else {
      const response = await api.post('/decks', newFormat);
      return response;
    }
  },
